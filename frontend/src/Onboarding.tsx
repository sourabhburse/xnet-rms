...
import { DeviceGroup, EnrollmentToken, Organization, Registration, TagItem, User, PendingDevice } from "./types";
...
export default function Onboarding({ user, onRefresh }: { user: User; onRefresh?: () => void }) {
  ...
  const refresh = async () => {
    ...
    const [nextPending, nextRegistrations, nextTags, nextGroups, nextTokens] = await Promise.all([
  ...
  }, [user.organization_id, isSuperAdmin, org, canManage]);
  ...
  const submit = (entries: Entry[]) => run(async () => {
    ...
    const response = await request<any[]>("registrations", "POST", { organization_id: org, rows: valid });
    setResults(response || []);
