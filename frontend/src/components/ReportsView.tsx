...
function Summary({ label, value, note, tone = "neutral" }: { label: string; value: string | number; note: string; tone?: "neutral" | "ok" | "warn" }) {
  ...
  return <div className="border-b border-r border-border p-5 last:border-r-0"><div className="font-mono text-[10px] uppercase tracking-[0.08em] text-muted-foreground">{label}</div><div className={`mt-2 font-display text-[25px] font-semibold leading-none tabular-nums ${valueClass}`}>{value}</div><div className="mt-2 text-[12px] text-muted-foreground">{note}</div></div>;
}
...
export default function ReportsView({ devices, groups, tags, selectedOrg }: { devices: Device[]; groups: DeviceGroup[]; tags: TagItem[]; selectedOrg: string }) {
  ...
  return (
    <div className="-m-6 min-h-full bg-card">
      ...
      <section>
        <div className="border-b border-border px-6 py-4"><h2 className="text-[14px] font-semibold">Report controls</h2><p className="mt-1 text-[12px] text-muted-foreground">All displayed values come from the telemetry query endpoint. Choose one metric to drive the chart and table.</p></div>
        <div className="grid gap-3 border-b border-border bg-secondary/20 px-6 py-4 sm:grid-cols-2 lg:grid-cols-5">
      ...
      {!report ? <div className="flex min-h-52 flex-col items-center justify-center gap-2 border-b border-border text-center"><BarChart3 className="size-5 text-muted-foreground/60" /><p className="text-[13px] font-medium">No report generated</p><p className="max-w-sm text-xs text-muted-foreground text-pretty">Choose a scope and reporting window, then generate a report to see actual telemetry values.</p></div> : <>
        <div className="grid grid-cols-2 sm:grid-cols-4"><Summary label="Data points" value={reportRows.length} note={`${report.resolution || report.bucket || resolution} aggregates`} /><Summary label="Devices" value={distinctDevices} note="Included in the result" /><Summary label="Alerts" value={alertCount} note={report.alerts?.current_state || "No rollup returned"} tone={alertCount ? "warn" : "neutral"} /><Summary label="Window" value={days === "custom" ? `${fromDate} – ${toDate}` : `${days} days`} note="Selected reporting period" /></div>
        <section>
          <div className="flex flex-wrap items-end justify-between gap-3 border-b border-border px-6 py-4"><div><h2 className="text-[14px] font-semibold">Telemetry results</h2><p className="mt-1 text-[12px] text-muted-foreground">{reportRows.length.toLocaleString()} metric rows in the selected scope.</p></div><div className="flex items-center border border-border"><button type="button" onClick={() => setActiveTab("telemetry")} className={`px-3 py-1.5 text-[11px] font-medium ${activeTab === "telemetry" ? "bg-primary text-primary-foreground" : "bg-card"}`}>Telemetry</button><button type="button" disabled className="border-l border-border px-3 py-1.5 text-[11px] text-muted-foreground">Availability unavailable</button></div></div>
          <div className="space-y-4 px-6 py-5">
            ...
            {chartPoints.length > 1 ? <div className="h-[260px] w-full"><ResponsiveContainer width="100%" height="100%"><LineChart data={chartPoints} margin={{ top: 8, right: 12, left: 4, bottom: 8 }}><CartesianGrid stroke="var(--row-divider)" vertical={false} /><XAxis dataKey="label" tick={{ fill: "var(--muted-foreground)", fontSize: 10 }} minTickGap={30} /><YAxis tick={{ fill: "var(--muted-foreground)", fontSize: 10 }} width={42} /><Tooltip contentStyle={{ background: "var(--popover)", border: "1px solid var(--border)", borderRadius: 0, fontSize: 12 }} formatter={(value: number) => [display(value, field?.unit), field?.label || fieldID]} /><Line type="monotone" dataKey="value" stroke="var(--primary)" strokeWidth={2} dot={false} /></LineChart></ResponsiveContainer></div> : <div className="flex min-h-44 items-center justify-center border border-dashed border-border text-xs text-muted-foreground">Not enough numeric samples for a trend chart.</div>}
            <div className="overflow-x-auto border border-border"><Table><TableHeader><TableRow><TableHead>Device</TableHead><TableHead>Bucket</TableHead><TableHead>Metric</TableHead><TableHead>Value</TableHead><TableHead>Range / delta</TableHead></TableRow></TableHeader><TableBody>{reportRows.map(({ item, id, aggregate }) => <TableRow key={`${item.device_id}-${item.source_id}-${item.bucket}-${id}`}><TableCell><div className="max-w-[220px] truncate text-[13px]">{item.device_name}</div><div className="font-mono text-[10px] text-muted-foreground">{item.source_id}</div></TableCell><TableCell className="whitespace-nowrap font-mono text-[10px]">{formatBucket(item.bucket)}</TableCell><TableCell>{aggregate.label || id}</TableCell><TableCell className="font-mono text-[11px]">{display(aggregateValue(aggregate), aggregate.unit)}</TableCell><TableCell className="font-mono text-[10px] text-muted-foreground">{aggregate.min != null || aggregate.max != null ? `${aggregate.min ?? "—"} – ${aggregate.max ?? "—"}` : aggregate.delta_samples ? `Δ ${aggregate.delta ?? 0}${aggregate.resets ? ` · ${aggregate.resets} resets` : ""}` : aggregate.state_seconds ? "State durations reported" : "—"}</TableCell></TableRow>)}{!reportRows.length && <TableRow><TableCell colSpan={5} className="h-28 text-center text-xs text-muted-foreground">No values returned for this metric.</TableCell></TableRow>}</TableBody></Table></div>
          </div>
        </section>
        <div className="border-t border-border bg-secondary/25 px-6 py-3 text-[12px] text-muted-foreground">Availability rollups, least-available devices, group comparisons, and scheduled reports are not shown because the current API does not return those datasets.</div>
      </>}
    </div>
  );
}
