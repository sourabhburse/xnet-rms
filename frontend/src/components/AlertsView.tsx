...
type AlertTab = "all" | "open" | "pending" | "acknowledged" | "resolved";
...
function statusTone(status: AlertItem["status"]) {
  if (status === "OPEN") return "text-down";
  if (status === "PENDING") return "text-warn";
  if (status === "ACKNOWLEDGED") return "text-primary";
  return "text-ok";
}
...
export default function AlertsView({ user, selectedOrg, onUnreadChange }: { user: User; selectedOrg: string; onUnreadChange: (count: number) => void }) {
  ...
  const visibleItems = useMemo(() => {
    ...
    return items.filter((item) => {
      const tabMatches = tab === "all" || item.status === tab.toUpperCase();
  ...
  return (
    <div className="-m-6 min-h-full bg-card">
      ...
      <div className="grid grid-cols-2 border-b border-border sm:grid-cols-5">
        {(["all", "open", "pending", "acknowledged", "resolved"] as AlertTab[]).map((value) => <button key={value} type="button" onClick={() => setTab(value)} className={`border-b-2 border-r border-border px-5 py-3 text-left last:border-r-0 ${tab === value ? "border-b-primary bg-accent/30" : "border-b-transparent hover:bg-secondary/35"}`}><span className="block font-mono text-[10px] uppercase tracking-[0.08em] text-muted-foreground">{value === "all" ? "All alerts" : value}</span><span className={`mt-1 block font-display text-[24px] font-semibold tabular-nums ${value === "open" && counts.open ? "text-down" : value === "pending" && counts.pending ? "text-warn" : "text-foreground"}`}>{counts[value]}</span>{value === "pending" && <span className="mt-1 block text-[10px] text-muted-foreground">Debouncing</span>}</button>)}
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-border bg-secondary/20 px-6 py-3">
          ...
          <span className="ml-auto font-mono text-[10px] uppercase tracking-[0.08em] text-muted-foreground">{visibleItems.length} visible</span>
      </div>

      <div className="grid xl:grid-cols-[minmax(0,1fr)_360px]">
        <section className="min-w-0 border-b border-border xl:border-b-0 xl:border-r">
          <div className="border-b border-border px-6 py-4"><h2 className="text-[14px] font-semibold">Alert queue</h2><p className="mt-1 text-[12px] text-muted-foreground">Acknowledgement assigns ownership; it does not resolve the reported condition.</p></div>
            <div className="overflow-x-auto">
              <Table><TableHeader><TableRow><TableHead>Condition</TableHead><TableHead>Severity</TableHead><TableHead>State</TableHead><TableHead>Last updated</TableHead><TableHead className="text-right">Action</TableHead></TableRow></TableHeader><TableBody>
                {visibleItems.map((item) => <TableRow key={item.id} className={`cursor-pointer border-l-2 ${selected?.id === item.id ? "border-l-primary bg-accent/30" : item.status === "OPEN" ? "border-l-down-rail" : item.status === "PENDING" ? "border-l-warn-rail" : item.status === "RESOLVED" ? "border-l-ok-rail" : "border-l-primary/60"} hover:bg-secondary/30`} onClick={() => void inspect(item)}>
                  ...
                  <TableCell><span className={`font-mono text-[10px] uppercase tracking-[0.08em] ${item.severity === "critical" ? "text-down" : "text-warn"}`}>{item.severity}</span></TableCell>
                  <TableCell><span className={`inline-flex items-center gap-1.5 font-mono text-[10px] uppercase tracking-[0.08em] ${statusTone(item.status)}`}><span className="size-1.5 rounded-full bg-current" />{statusLabel(item.status)}</span></TableCell>
        ...
        <aside className="h-fit bg-secondary/15">
          <div className="border-b border-border px-5 py-4"><h2 className="text-[14px] font-semibold">Alert details</h2><p className="mt-1 text-[12px] text-muted-foreground">{selected ? `${selected.template_name} · v${selected.template_version}` : "Select an alert to inspect its lifecycle."}</p></div>
          <div className="space-y-5 p-5">
            {selected ? <>
              <div className="border-l-2 border-l-primary pl-3"><div className="flex items-start justify-between gap-2"><div><div className="text-[13px] font-semibold">{selected.device_name}</div><div className="mt-1 font-mono text-[10.5px] text-muted-foreground">{selected.metric_id}{selected.entity_key ? ` / ${selected.entity_key}` : ""}</div></div><span className={`font-mono text-[10px] uppercase tracking-[0.08em] ${statusTone(selected.status)}`}>{statusLabel(selected.status)}</span></div><div className="mt-4 grid grid-cols-2 gap-3 border-t border-border pt-3 text-[11px]"><div><span className="block text-muted-foreground">Severity</span><span className={`mt-1 block font-mono uppercase ${selected.severity === "critical" ? "text-down" : "text-warn"}`}>{selected.severity}</span></div><div><span className="block text-muted-foreground">Last value</span><span className="mt-1 block font-mono">{formatValue(selected.last_value)}</span></div></div></div>
      ...
      <div className="border-t border-border bg-secondary/25 px-6 py-3 text-[12px] text-muted-foreground">Trend charts and rule editing are not shown because the current API exposes alert state and lifecycle events, but not historical chart series or rule mutation endpoints.</div>
    </div>
  );
}
