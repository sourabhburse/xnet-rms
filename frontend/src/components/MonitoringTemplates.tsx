...
import { CatalogMetric, Device, DeviceGroup, MonitoringBinding, MonitoringCatalog, MonitoringMetricSelection, MonitoringTemplate, MonitoringTemplateDefinition, Organization, TagItem, User } from "../types";
...
const safeDefinition = (item: MonitoringTemplate): MonitoringTemplateDefinition => {
  const definition = item?.definition;
  return {
    catalog_version: definition?.catalog_version ?? 1,
    interval_seconds: definition?.interval_seconds ?? 60,
    metrics: Array.isArray(definition?.metrics) ? definition.metrics : [],
    stale: definition?.stale ? { ...definition.stale } : { enabled: false },
  };
};
...
export default function MonitoringTemplates({ currentUser, data, organizations, groups, tags, selectedOrg, loading, onRefresh }: Props) {
  ...
  const openEdit = (item: MonitoringTemplate) => {
    const definition = safeDefinition(item);
    ...
    setEditing(item); setName(item.name); setDescription(item.description); setIntervalSeconds(definition.interval_seconds); setSelected(definition.metrics.map(metric => metric.metric_id)); setLabels(nextLabels); setGauges(nextGauges); setStates(nextStates); setStale(definition.stale.enabled); setStaleSeverity(definition.stale.severity || "warning"); setPreview([]); setEditorOpen(true);
  };
