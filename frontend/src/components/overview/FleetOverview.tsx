  ...
  return (
    <div className="flex min-h-full flex-col bg-card">
      <section className="flex flex-wrap items-end justify-between gap-4 border-b border-border px-6 pb-[18px] pt-6">
        <div>
          <h1 className="font-display text-[22px] font-semibold tracking-[-0.01em] text-foreground">Overview</h1>
        ...
        {isOrgAdmin && <div className="flex gap-2"><Button size="sm" onClick={() => onNavigate("onboarding")}><Plus />Onboard devices</Button></div>}
      </section>
      ...
      <section className="grid grid-cols-2 border-b border-border bg-card md:grid-cols-5">
        ...
        <KpiTile label="Onboarding" value={onboarding} tone="warn" caption={`${awaitingCount} awaiting · ${pendingCount} unclaimed`} onClick={() => onNavigate("onboarding")} />
