...
func TestPostgresLifecycle(t *testing.T) {
	...
	legacyScoped := Actor{ID: viewer, Role: "ORG_ADMIN"}
	assertEmpty := func(name string, h http.HandlerFunc) {
		t.Helper()
		r := httptest.NewRequest("GET", "http://rms.test/", nil).WithContext(context.WithValue(context.Background(), actorKey{}, legacyScoped))
		w := httptest.NewRecorder()
		h(w, r)
		if w.Code != 200 {
		...
		if e := json.Unmarshal(w.Body.Bytes(), &rows); e != nil || len(rows) != 0 {
			t.Fatalf("%s leaked rows: %s", name, w.Body.String())
		}
	}
	assertEmpty("users", core.users)
	assertEmpty("tokens", core.tokens)
	assertEmpty("audit logs", core.auditLogs)
	assertEmpty("sessions", core.sessions)
	w = httptest.NewRecorder()
	core.listDevices(w, httptest.NewRequest("GET", "http://rms.test/", nil).WithContext(context.WithValue(context.Background(), actorKey{}, legacyScoped)))
	var devices struct {
		Total int `json:"total"`
	}
	if w.Code != 200 || json.Unmarshal(w.Body.Bytes(), &devices) != nil || devices.Total != 0 {
		t.Fatalf("devices leaked rows: %d %s", w.Code, w.Body.String())
	}
	w = httptest.NewRecorder()
	core.dashboard(w, httptest.NewRequest("GET", "http://rms.test/", nil).WithContext(context.WithValue(context.Background(), actorKey{}, legacyScoped)))
	var dashboard map[string]int
	if w.Code != 200 || json.Unmarshal(w.Body.Bytes(), &dashboard) != nil || dashboard["total"] != 0 {
		t.Fatalf("dashboard leaked rows: %d %s", w.Code, w.Body.String())
	}
	w = httptest.NewRecorder()
	core.profiles(w, httptest.NewRequest("GET", "http://rms.test/", nil).WithContext(context.WithValue(context.Background(), actorKey{}, legacyScoped)))
	if w.Code != 200 || strings.Contains(w.Body.String(), org) {
		t.Fatalf("profiles leaked tenant rows: %d %s", w.Code, w.Body.String())
	}
	acks := 0
...
func TestFreshInstallBootstrapDataContract(t *testing.T) {
	dsn := os.Getenv("RMS_TEST_DATABASE_URL")
	if dsn == "" {
		t.Skip("set RMS_TEST_DATABASE_URL for PostgreSQL integration tests")
	}
	...
	db.SetMaxOpenConns(1)
	db.SetMaxIdleConns(1)
	schema := "rms_bootstrap_test_" + randomID()
	if _, e = db.Exec("CREATE SCHEMA " + schema); e != nil {
		t.Fatal(e)
	}
	defer db.Exec("DROP SCHEMA " + schema + " CASCADE")
	...
	if e = Partitions(db, time.Now().UTC()); e != nil {
		t.Fatal(e)
	}
	for _, table := range freshInstallDataTables {
		...
		if count != 0 {
			t.Fatalf("fresh migration seeded %s with %d rows", table, count)
		}
	}

	if e = BootstrapOrgAdmin(db, "owner@example.com", "local-bootstrap-password", "Example Organization"); e != nil {
		t.Fatal(e)
	}
	var organizations, users int
	if e = db.QueryRow("SELECT count(*) FROM organizations").Scan(&organizations); e != nil {
		t.Fatal(e)
	}
	if e = db.QueryRow("SELECT count(*) FROM users").Scan(&users); e != nil {
		t.Fatal(e)
	}
	if organizations != 1 || users != 1 {
		t.Fatalf("bootstrap created organizations=%d users=%d, want 1 and 1", organizations, users)
	}
	var role, organization string
	if e = db.QueryRow("SELECT role,organization_id FROM users WHERE email='owner@example.com'").Scan(&role, &organization); e != nil {
		t.Fatal(e)
	}
	if role != "ORG_ADMIN" || organization == "" {
		t.Fatalf("bootstrap account role=%q organization=%q", role, organization)
	}
	for _, table := range freshInstallDataTables {
		if table == "organizations" || table == "users" {
			continue
		}
		var count int
		if e = db.QueryRow("SELECT count(*) FROM " + table).Scan(&count); e != nil {
		...
		if count != 0 {
			t.Fatalf("bootstrap seeded %s with %d rows", table, count)
		}
	}
