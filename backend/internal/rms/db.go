...
func Migrate(d *sql.DB) error {
	...
	for version, name := range []string{"001_initial.sql", "002_bundles.sql", "003_audit.sql", "004_onboarding.sql", "005_ssh_luci.sql", "006_groups.sql", "007_device_overview.sql", "008_profile_ownership.sql", "009_monitoring_alerts.sql", "010_user_organization.sql"} {
		var n int
...
func CheckSchema(d *sql.DB) error {
	var n int
	e := d.QueryRow("SELECT count(*) FROM rms_migrations WHERE version=10").Scan(&n)
	if e != nil || n != 1 {
