...
func Migrate(d *sql.DB) error {
	...
	for version, name := range []string{"001_initial.sql", "002_bundles.sql", "003_audit.sql", "004_onboarding.sql"} {
		var n int
...
func CheckSchema(d *sql.DB) error {
	var n int
	e := d.QueryRow("SELECT count(*) FROM rms_migrations WHERE version=4").Scan(&n)
	if e != nil || n != 1 {
