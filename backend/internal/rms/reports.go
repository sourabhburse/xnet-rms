...
func reportScopeSQL(alias string) string {
	return `($4='organization' OR ($4='device' AND ` + alias + `.id=$5)
		OR ($4='group' AND EXISTS(SELECT 1 FROM device_group_members gm WHERE gm.device_id=` + alias + `.id AND gm.group_id=$5))
		OR ($4='tag' AND EXISTS(SELECT 1 FROM device_tags dt WHERE dt.device_id=` + alias + `.id AND dt.tag_id=$5)))`
}
...
func (s *Core) runTelemetryReport(r *http.Request, req *telemetryReportRequest) (telemetryReportResult, error) {
	...
	query := `SELECT h.device_id,coalesce(nullif(d.name,''),d.serial_number),h.source_id,h.hour,h.fields
		...
		WHERE ($1 OR d.organization_id=$2) AND $3::text IS NOT NULL AND NOT d.revoked AND h.hour >= $6 AND h.hour < $7 AND ` + reportScopeSQL("d") + `
		ORDER BY h.hour,h.device_id,h.source_id LIMIT $8 OFFSET $9`
	rows, err := s.DB.Query(query, allOrgs, org, req.TemplateID, req.ScopeType, req.ScopeID, from, to, req.PageSize+1, offset)
	if err != nil {
		return telemetryReportResult{}, err
	}
	defer rows.Close()
	...
	if offset == 0 && (len(selected) == 0 || selected["availability"] != nil) {
		...
		WHERE ($1 OR d.organization_id=$2) AND $3::text IS NOT NULL AND NOT d.revoked AND p.hour >= $6 AND p.hour < $7 AND ` + reportScopeSQL("d") + ` ORDER BY p.hour,p.device_id`
		prows, err := s.DB.Query(presenceQuery, allOrgs, org, req.TemplateID, req.ScopeType, req.ScopeID, from, to)
		if err != nil {
			return telemetryReportResult{}, err
		}
		for prows.Next() {
	...
	alertQuery := `SELECT count(*),count(*) FILTER(WHERE a.severity='warning'),count(*) FILTER(WHERE a.severity='critical'),
		...
		FROM alerts a JOIN devices d ON d.id=a.device_id WHERE ($1 OR a.organization_id=$2) AND ($3='' OR a.template_id=$3)
		AND ` + reportScopeSQL("d") + ` AND coalesce(a.opened_at,a.created_at)<$7 AND coalesce(a.resolved_at,$7)>$6`
	if err = s.DB.QueryRow(alertQuery, allOrgs, org, req.TemplateID, req.ScopeType, req.ScopeID, from, to).Scan(&count, &warningCount, &criticalCount, &first, &last, &current); err != nil {
		return telemetryReportResult{}, err
	}
	durationQuery := `WITH timeline AS (
		...
		WHERE ($1 OR a.organization_id=$2) AND ($3='' OR a.template_id=$3) AND ` + reportScopeSQL("d") + `
	) SELECT
		coalesce(sum(EXTRACT(epoch FROM (least(coalesce(next_at,$7),$7)-greatest(occurred_at,$6)))) FILTER(WHERE severity='warning' AND event IN ('opened','acknowledged','escalated') AND occurred_at<$7 AND coalesce(next_at,$7)>$6),0),
		coalesce(sum(EXTRACT(epoch FROM (least(coalesce(next_at,$7),$7)-greatest(occurred_at,$6)))) FILTER(WHERE severity='critical' AND event IN ('opened','acknowledged','escalated') AND occurred_at<$7 AND coalesce(next_at,$7)>$6),0)
		FROM timeline`
	if err = s.DB.QueryRow(durationQuery, allOrgs, org, req.TemplateID, req.ScopeType, req.ScopeID, from, to).Scan(&warningSeconds, &criticalSeconds); err != nil {
		return telemetryReportResult{}, err
	}
...
func (s *Core) telemetryReport(w http.ResponseWriter, r *http.Request) {
	...
	allOrgs := a.AllOrgs && org == ""
	query := `SELECT h.device_id,coalesce(d.name,d.serial_number),h.source_id,h.hour,h.fields
	FROM hourly_summaries h JOIN devices d ON d.id=h.device_id
	WHERE ($1 OR d.organization_id=$2) AND NOT d.revoked
	AND ($3='' OR h.device_id=$3)
	AND ($4='' OR EXISTS(SELECT 1 FROM device_group_members gm JOIN device_groups g ON g.id=gm.group_id WHERE gm.device_id=h.device_id AND g.id=$4))
	AND ($5='' OR EXISTS(SELECT 1 FROM device_tags dt WHERE dt.device_id=h.device_id AND dt.tag_id=$5))
	AND ($6='' OR h.source_id=$6) AND h.hour >= $7 AND h.hour <= $8
	ORDER BY h.hour,h.device_id,h.source_id LIMIT 10000`
	rows, err := s.DB.Query(query, allOrgs, org, deviceID, groupID, tagID, source, from, to)
	if err != nil {
