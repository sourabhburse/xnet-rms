...
func (s *Core) alerts(w http.ResponseWriter, r *http.Request) {
	...
	items, err := jsonRows(s.DB, `SELECT row_to_json(x) FROM (
		...
		WHERE ($1 OR a.organization_id=$2) AND ($3='' OR a.status=$3) AND ($4='' OR a.severity=$4) AND ($5='' OR a.device_id=$5)
	...
	) x`, a.AllOrgs, org, status, severity, deviceID)
	if err != nil {
		fail(w, 503, "alerts unavailable")
		return
	}
	var unread int
	if err = s.DB.QueryRow("SELECT count(*) FROM alerts WHERE ($1 OR organization_id=$2) AND status IN ('PENDING','OPEN')", a.AllOrgs, org).Scan(&unread); err != nil {
		fail(w, 503, "alerts unavailable")
		return
	}
