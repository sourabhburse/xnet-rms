...
static void write_status(void){
 ...
 json_object_set_string(o,"last_error",error_code);json_object_set_string(o,"serial_number",g_cfg.serial);json_object_set_string(o,"lan_mac",g_cfg.mac_address);
 JSON_Value *ids=g_cfg.identifiers[0]?json_parse_string(g_cfg.identifiers):json_value_init_object();if(!ids||json_value_get_type(ids)!=JSONObject){if(ids)json_value_free(ids);ids=json_value_init_object();}if(strlen(g_cfg.mac_address)==17)json_object_set_string(json_value_get_object(ids),"mac",g_cfg.mac_address);json_object_set_value(o,"identifiers",ids);
 json_object_set_string(o,"model",g_cfg.model);json_object_set_string(o,"firmware_version",g_cfg.firmware_version);json_object_set_string(o,"agent_version",AGENT_VERSION);json_object_set_string(o,"device_id",g_cfg.device_id);json_object_set_string(o,"organization_name",g_cfg.organization);
