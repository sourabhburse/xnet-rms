...
void detect_board_hardware(void)
{
    ...
    if (!g_cfg.mac_address[0])
    {
        ...
        {
            struct uci_section *lan = uci_lookup_section(ctx, pkg, "lan");
            ...
            if (!dev && (!type || strcmp(type, "bridge")))
                dev = lan ? uci_lookup_option_string(ctx, lan, "ifname") : NULL;
        ...
        if (f)
        {
            if (fgets(g_cfg.mac_address, sizeof(g_cfg.mac_address), f))
                g_cfg.mac_address[strcspn(g_cfg.mac_address, "\r\n")] = 0;
            fclose(f);
        }
    }
...
int generate_ec_p256_key_if_missing(void)
{
    ...
    {
        FILE *f = fopen(RMS_CLIENT_KEY, "r");
        if (!f)
            return -1;
        EC_KEY *key = PEM_read_ECPrivateKey(f, NULL, NULL, NULL);
        fclose(f);
        int ok = key && EC_KEY_check_key(key) == 1;
        EC_KEY_free(key);
    ...
    if (b && EC_KEY_generate_key(key) == 1 && PEM_write_bio_ECPrivateKey(b, key, NULL, NULL, 0, NULL, NULL) == 1)
    {
        ...
        rc = rms_write_atomic(RMS_CLIENT_KEY, data, n, 0600);
    }
    BIO_free(b);
    EC_KEY_free(key);
    return rc;
...
static int install_certificate(const char *text, const char *id)
{
    ...
    if (!cert)
        return -1;
    FILE *f = fopen(RMS_CLIENT_KEY, "r");
    EVP_PKEY *key = f ? PEM_read_PrivateKey(f, NULL, NULL, NULL) : NULL;
    if (f)
        fclose(f);
    char cn[128] = {0};
...
int rms_cert_due(void)
{
    ...
    int due = X509_cmp_time(X509_get0_notAfter(x), &now) <= 0 ? 2 : X509_cmp_time(X509_get0_notAfter(x), &renew) <= 0 ? 1
                                                                                                                      : 0;
    if (X509_cmp_time(X509_get0_notBefore(x), &now) > 0)
        due = 2;
    X509_free(x);
    return due;
}
...
int perform_challenge_recovery(void)
{
    if (!rms_id(g_cfg.device_id))
        return -1;
    char url[512];
    snprintf(url, sizeof(url), "%s/api/v1/provision/challenge", g_cfg.server_url);
    JSON_Value *req = json_value_init_object();
    json_object_set_string(json_value_get_object(req), "device_id", g_cfg.device_id);
    char *b = json_serialize_to_string(req);
    json_value_free(req);
    char *resp = rms_http(url, b, 0, 8192);
    free(b);
    if (!resp)
        return -1;
    JSON_Value *ch = json_parse_string(resp);
    free(resp);
    JSON_Object *o = json_value_get_object(ch);
    const char *message = json_object_get_string(o, "message"), *id = json_object_get_string(o, "challenge_id");
    char prefix[128];
    snprintf(prefix, sizeof(prefix), "xnet-rms/recovery/v1:%s:", g_cfg.device_id);
    if (!message || strncmp(message, prefix, strlen(prefix)) || !rms_id(id))
    {
        json_value_free(ch);
        return -1;
    }
    char *sig = sign_challenge_message(message);
    if (!sig)
    {
        json_value_free(ch);
        return -1;
    ...
    free(sig);
    json_value_free(ch);
    b = json_serialize_to_string(req);
    json_value_free(req);
    snprintf(url, sizeof(url), "%s/api/v1/provision/recover", g_cfg.server_url);
    resp = rms_http(url, b, 0, 8192);
    free(b);
    if (!resp)
        return -1;
    req = json_parse_string(resp);
    free(resp);
    int rc = install_certificate(json_object_get_string(json_value_get_object(req), "certificate"), g_cfg.device_id);
    json_value_free(req);
    return rc;
}
...
{
    int due = rms_cert_due();
    if (due == 0)
        return 0;
    if (due == 2)
        return perform_challenge_recovery();
    char url[512];
    snprintf(url, sizeof(url), "%s/api/v1/provision/renew", g_cfg.server_url);
    char *resp = rms_http(url, "{}", 1, 8192);
    if (!resp)
        return -1;
    JSON_Value *v = json_parse_string(resp);
    free(resp);
    int rc = install_certificate(json_object_get_string(json_value_get_object(v), "certificate"), g_cfg.device_id);
    json_value_free(v);
    return rc;
}
